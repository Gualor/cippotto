...
chip8_err_t chip8_execute(uint16_t opcode)
{
    ...
    {
        printf("RET\n");
        if (SP == 0) return CHIP8_EXIT;
        PC = stack[SP--];
    }
    else if (O___ == 0x1000)
    {
        printf("JP 0x%04X\n", _NNN);
        if (_NNN > RAM_ADDR_END) return CHIP8_INVALID_ADDR;
        PC = _NNN;
    }
    else if (O___ == 0x2000)
    {
        printf("CALL 0x%04X\n", _NNN);
        if (SP == STACK_SIZE - 1) return CHIP8_STACK_OVERFLOW;
        stack[++SP] = PC;
    }
    else if (O___ == 0x3000)
    {
...
{
    PC += sizeof(uint16_t);
    if (PC > RAM_ADDR_END) return CHIP8_INVALID_ADDR;
    return CHIP8_OK;
}
